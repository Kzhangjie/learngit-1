...
from airsheet.util import parse_range_to_index
...
class Range:
    ...
    def set_none_value_with_others(self, other: "Range") -> "Range":
        return Range(
            ...
            columns_from=(
                self.columns_from
                if self.columns_from is not None
                else other.columns_from
            ),
            columns_to=(
                self.columns_to if self.columns_to is not None else other.columns_to
            ),
        )
...
class Client:
    def __init__(self, wps_sid: str, weboffice_branch: str):
        ...
        if weboffice_branch:
            session.cookies.set(
                "weboffice_branch", weboffice_branch, domain=".kdocs.cn"
            )
    ...
    def get_range_cells(self, file_id: str, sheet_id: int, range: Range) -> List[Cell]:
        ...
        try:
            ...
            # 排序，确保每一列的行、列的顺序是对的
            return sorted(
                cells, key=lambda x: (x.range.rows_from, x.range.columns_from)
            )
        except:
    ...
    def add_worksheet(
        ...
        if sheet_name is not None and sheet_name != "":
            self.update_worksheet_name(
                file_id=file_id, sheet_id=new_sheet_id, sheet_name=sheet_name
            )

        return new_sheet_id

    def update_worksheet_name(
        self, file_id: str, sheet_id: int, sheet_name: str
    ) -> None:
        self.do_core_exec(
            file_id=file_id,
            args={
