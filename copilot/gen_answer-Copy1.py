...
def run_one(self,row):
    ...
    try:
        ...
        assistant_contents = [item["content"] for item in data_list if item.get("role") == "assistant"]
        third_last_content = assistant_contents[-3]
        second_last_content = assistant_contents[-2]
        content = third_last_content + "\n" +  second_last_content
        # content = next((
        ...
        # codes_in_logs = ''
        airsheet.write_xl([f'\'{s_id}',r1,r2,r3,r4,content], f'{self.clo_num_to_write}{row["row_index"]}', sheet_name=self.sheetname)
