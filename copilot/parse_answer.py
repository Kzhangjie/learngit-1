...
def run_one(self,row):
    try:
        ...
        logs = json.loads(row["LOGS"]+row.get("LOGS1","")+row.get("LOGS2","")+row.get("LOGS3",""))
        codes_in_logs = extra_codes_from_logs(logs)
        # codes_in_logs = ''
        # return [text,codes,codes_in_logs]
        return [codes,codes_in_logs]
