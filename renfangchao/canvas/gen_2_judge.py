...
def find_first_difference(a, b):
    ...
    while i < len(a) and j < len(b):
        if a[i] != b[j]:
            # 如果b[j]是websearch且与a[i]不同，跳过b[j]
            if b[j] == "websearch":
                j += 1
                continue
            return b[j]
...
def judge(expected_funcs, grouped_functions, aim):
    ...
    for i, ef in enumerate(expected_funcs):
        ...
        if i < len(grouped_functions):
            gf = grouped_functions[i]
            # gf 中存在连续的websearch时，合并
            if "websearch" in gf:
                new_gf = []
                for f in gf:
                    if f == "websearch":
                        if new_gf and new_gf[-1] == "websearch":
                            continue
                    new_gf.append(f)
                gf = new_gf
        else:
        ...
        if aim:
            if any([e in aim for e in ef]):
                print(f"expect_str: {expect_str}, code: {code}")
                if aim[0] in ["generate_mindmap", "generate_ppt"]:
                    ef = [e for e in ef if e != "websearch"]
                    gf = [e for e in gf if e != "websearch"]
                    expect_str = " ".join(ef)
                    code = " ".join(gf)
                if aim[0] in ["chat", "websearch"]:
                    expect_str = expect_str.replace("chat", "websearch")
                    code = code.replace("chat", "websearch")
                print(f"expect_str: {expect_str}, code: {code}")
                if expect_str.startswith("#"):
                    is_pass = expect_str[:1] in code
                else:
                    is_pass = code == expect_str
                if not is_pass:
                    if i < len(grouped_functions):
                        one_result = find_first_difference(ef, gf)
                    else:
...
def run_one(self, row):
    ...
    is_aim_pass, one_result = try_judge(expected_funcs, codes, aim)
    ...
    # rts = [r[0] for r in rs]
    # rss = "\n\n".join(
    #     [f"_______________req{i+1}_______________\n\n{rt}" for i, rt in enumerate(rts)]
    # )
    airsheet.write_xl(
        [is_aim_pass, one_result],
        f'M{row["row_index"]}',
        sheet_name=self.sheetname,
    )
