...
# sheetname = "ppt"
必须有的列名 = ["问题"]
输出列名 = "d5_reason"  # 表示会跳过"答案"列已经有内容的行。
# 输出列名 = ""
输出列编号 = "E"  # 表示从把答案写到B列
...
from mangping.readfile.prompts import row_to_prompt


def run_one(self, row):
    qs = [row_to_prompt(row)]
    ...
    is_error = False
    for r in rs:
        if "event:error" in r[0]:
            is_error = True
            print(f"Error in response: {r[0]}")
            traceback.print_exc()
            break
    session_id = str(session_id)
    success = "success" if not is_error else "fail"
    results = [
        success,
