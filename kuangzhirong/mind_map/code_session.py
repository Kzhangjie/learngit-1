...
from mind_map.utils.cutting import Cutter
import re
...
def trans_chat_messages_2_messages(messages: List[dict],qs: List[str]):
    ...
    chat_messages.append(
        {"role": "system", "type": "text", "content": f"今天是:{systemtoday}。"}
    )
    index =0
    for x in messages:
        if not x.get("role"):
        ...
        if x.get("role") == "user" and x.get("type") == "text":
            ...
            elif typec in {"text", "execution", "st_text"}:
                if websearch_result:
                    try:
            ...
            elif typec in {"long_writer", "ppt_outline_text","mind_map"}:
                websearch_result = ""
                chat_messages.append(
                    {"role": x.get("role"), "type": "execution", "content": x.get("content")}
                )
...
def history_to_model_messages(history: dict,fileinfo):
    ...
    for message in messages:
        ...
        if message["role"] == "assistant" and message["type"] in {"websearch_result","deepsearch_result"} and websearchquery :
            ...
            #     )
        elif message["role"] == "assistant" and message["type"] in {"text", "execution","long_writer", "ppt_outline_text","mind_map"}:
            new_messages.append({
                "role": message["role"],
                "name": "WPS 灵犀",
...
def chat_stream(messages: List[dict],chat_url = "", parameters = {"temperature": 0.01, "top_p": 0.01,"top_k":1}):
    ...
    try:
        ...
        if resp.headers.get("Content-Type") == "application/json":
            print("sse流程中出现error！error原因：",resp.text)
