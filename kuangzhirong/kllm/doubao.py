...
class admin:
    ...
    def messages(self, session_id: str) -> list[AdminMessage]:
        ...
        data = self.__request('GET', f'https://copilot.wps.cn/api/aigc/admin/cc/sessions/{session_id}/messages',
                              params=params)
        if not data['list']:
            return []
...
class planning:
    def __init__(self,
                 url: str = 'http://120.92.122.107:30818/kas/ybsjuqhgzi9-qkatvhtcdydnav5x/api/chat/completions?model=latest',
                 model: str = 'copilot365'):
        self.__url = url
        self.__model = model
...
def fetch_website(url: str, timeout: float = 5.0) -> URLInfo:
    ...
    req = requests.Request(method='post', url='http://120.92.124.158/office/v5/dev/ai/crawler/crawl',
